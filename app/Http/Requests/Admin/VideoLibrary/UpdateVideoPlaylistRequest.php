...
class UpdateVideoPlaylistRequest extends BaseFormRequest
{
    ...
    public function rules(): array
    {
        ...
        $playlist = $this->route('videoPlaylist');
        $locale = $this->input('locale', $playlist->locale);

        return [
            'title' => ['sometimes', 'string', 'min:2', 'max:200'],
            'locale' => ['sometimes', 'string', Rule::in(VideoPlaylist::LOCALES)],
            ...
            'slug' => [
                ...
                Rule::unique('video_playlists', 'slug')->where(fn ($q) => $q->where('locale', $locale))->ignore($playlist->id),
            ],
