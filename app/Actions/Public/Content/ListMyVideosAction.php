...
/**
 * قائمة فيديوهات الكاتب نفسه (نطاق عام — V1).
 *
 * تُرجع كل فيديوهات الكاتب بكل الحالات (draft/submitted/.../rejected) ليرى حالة
 * ما أرسله. غير قابلة للكاش (per-user، تحوي مسودّات خاصّة). القراءة محصورة
 * بـ author_id = الفاعل — لا يرى محتوى غيره. يعيد استخدام VideoResource
 * (نفس عقد الإنشاء/الإرسال في V1) للاتساق وإظهار status.
 */
class ListMyVideosAction
{
    public function handle(User $actor, Request $request): JsonResponse
    {
        ...
        return ApiResponse::success(
            data: VideoResource::collection($paginator)->resolve(),
            meta: ['pagination' => [
